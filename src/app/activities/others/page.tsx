...
interface ActivitiesOthersPageProps {
  searchParams?: {
    page?: string;
  };
}

export default async function ActivitiesOthersPage({ searchParams }: ActivitiesOthersPageProps) {
  ...
  return (
    <PageShell>
      <PageEnter>
        <PageHeader
          title="特色活动 · 其他"
          subtitle="涵盖工作坊、跨院系联动、公益实践等更多板块，集中展示多元文化交流。"
        />

        <Section
          description="展示“活动-其他活动”类别推文，未来可扩展更多类型的专题活动。"
          title="活动推文 / Special Events"
        >
          {resources.length === 0 ? (
            <Panel className="border-dashed text-sm text-ink/60">暂无相关推文，待后台录入后自动展示。</Panel>
          ) : (
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {resources.map((item) => (
                ...
                >
                  ...
                  <p className="mt-3 flex-1 text-sm leading-relaxed text-ink/70">{item.summary || "点击查看完整活动报道。"}</p>
                </Link>
              ))}
            </div>
          )}
          ...
          <div className="mt-8 flex items-center justify之间">
            {page <= 1 ? (
              ...
              <Button asChild variant="outline">
                <Link prefetch={false} href={`/activities/others?page=${page - 1}`} scroll={false}>
                  上一页
                </Link>
              </Button>
            )}
