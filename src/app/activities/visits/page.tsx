...
interface ActivitiesVisitsPageProps {
  searchParams?: {
    page?: string;
  };
}

export default async function ActivitiesVisitsPage({ searchParams }: ActivitiesVisitsPageProps) {
  ...
  return (
    <PageShell>
      <PageEnter>
        <PageHeader
          title="特色活动 · 访学交流"
          subtitle="记录国内外访学、实地走访与沉浸式研习的精彩瞬间。"
        />

        <Section
          description="聚焦“活动-访学交流”类型推文，每次访问均从数据库实时获取最新内容。"
          title="访学交流推文 / Study Visits"
        >
          <Suspense fallback={<ListFallback message="访学交流内容加载中..." />}>
            <VisitsFeed page={page} />
          </Suspense>
        </Section>
      </PageEnter>
    </PageShell>
...
const VisitsFeed = async ({ page }: { page: number }) => {
  ...
  if (resources.length === 0) {
    return <Panel className="border-dashed text-sm text-ink/60">暂无访学推文，上传后将自动展示最近内容。</Panel>;
  }
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        {resources.map((item) => (
          ...
          >
            ...
            <p className="mt-3 flex-1 text-sm leading-relaxed text-ink/70">{item.summary || "点击查看完整访学记录。"}</p>
          </Link>
        ))}
      </div>

      <Pagination page={page} hasNext={hasNext} basePath="/activities/visits" />
    </>
  );
};
