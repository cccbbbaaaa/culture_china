...
- 当前页面作为官网公开版本的一部分继续对外展示，并明确标记为“待建设 / 持续建设中”
- 后续在现有公开页面基础上持续补齐正式内容与 metadata，保持栏目持续建设

* [x] 决定 `课程介绍` 当前策略：保留栏目与入口，并以公开版本持续建设
* [x] 保留 Header 中 `课程介绍` 导航入口
* [x] 保留 `/curriculum/overview` 独立页面路由
* [x] 将 `课程介绍` 页面优化为可公开访问的“待建设”状态页
* [ ] 在当前公开页面基础上持续补齐 `课程介绍` 的正式内容与 metadata
...
**已处理 / 待跟进项**：
- `[已统一]` `src/app/page.tsx`、`src/app/intro/purpose/page.tsx`、`src/app/admissions/page.tsx` 中累计学员总数已统一按 `519` 处理
- `[当前基线]` `src/app/intro/page.tsx`、`src/app/intro/mission/page.tsx` 现使用 `17 期 / 519 / 459` 这一组口径
- `[待长期收口]` 若后续仍需频繁展示统计数字，建议抽成共享常量，避免再次出现页面分叉

**建议做法**：
- 先以旧官网/官方材料确定唯一口径
- 修正文案后，尽量抽到共享常量或内容配置，避免后续再次漂移

> 当前补充说明：用户已明确官方累计学员数按 `519` 处理，首页、培养宗旨、招生信息中的旧口径已完成替换。

* [ ] 确认官方口径：培养期数、累计学员、已结业人数、升学/职业去向比例
* [x] 统一首页、培养宗旨、招生信息等页面中的累计学员总数为 `519`
